...
Skills dispatch with `Subagent (general-purpose):` and either reference a prompt-template file (e.g., `superpower-subagent-driven-development`'s `./implementer-prompt.md`) or supply an inline prompt. On Gemini CLI:
...
| References `superpower-requesting-code-review`'s `./code-reviewer.md` | `invoke_agent` with `agent_name: "generalist"` and the filled review template |
